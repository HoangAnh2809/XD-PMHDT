...
async def get_current_user_info(request: Request, db: Session = Depends(get_db)):
    logger.info("Auth me request from %s headers=%s",
                request.client.host if request.client else '<unknown>', dict(request.headers))
...
async def proxy_service_center(path: str, request: Request):
    ...
    if auth_header:
        logger.info("Proxying to service_center %s from %s auth_preview=%s", url,
                    request.client.host if request.client else '<unknown>', auth_header[:30])
    else:
        logger.info("Proxying to service_center %s from %s (no auth header)",
                    url, request.client.host if request.client else '<unknown>')
    headers = {"Authorization": auth_header} if auth_header else {}
    params = dict(request.query_params)
...
async def proxy_chat_service(path: str, request: Request):
    ...
    try:
        ...
        content = {"error": "Invalid response from chat service",
...
async def proxy_payment_service(path: str, request: Request):
    ...
    if auth_header:
        logger.info("Proxying to payment %s from %s auth_preview=%s", url,
                    request.client.host if request.client else '<unknown>', auth_header[:30])
    else:
        logger.info("Proxying to payment %s from %s (no auth header)",
                    url, request.client.host if request.client else '<unknown>')
